"""
使用OpenPi策略控制Piper机器人，每次重置后等待用户输入任务。

示例用法：
```bash
python lerobot/scripts/run_piper_with_openpi.py \
    --host=localhost \
    --port=8000 \
    --device=cuda
```
...
from lerobot.common.policies.openpi.modeling_openpi import OpenPiPolicy

@dataclass
class PiperOpenPiConfig:
    """Piper OpenPi运行配置"""
    ...
    n_action_steps: int = 25  # OpenPi的动作步数
    
@safe_disconnect
def run_piper_with_openpi(
    robot: Robot,
    policy,
    device,
    use_amp=False,
    ...
    """
    运行Piper机器人，使用OpenPi策略进行控制，每次重置后等待用户输入任务
    """
    ...
    try:
        while not events["stop_running"]:
            ...
            if not task_description:
                task_description = "tidy up the items on the table"
...
def main():
    ...
    # 创建参数解析器
    parser = argparse.ArgumentParser(description="使用OpenPi策略控制Piper机器人")
    parser.add_argument("--host", type=str, default="localhost", help="OpenPi服务器主机地址")
    parser.add_argument("--port", type=int, default=8000, help="OpenPi服务器端口")
    ...
    parser.add_argument("--n_action_steps", type=int, default=25, help="动作步数")
    ...
    # 创建OpenPi配置
    openpi_config = PiperOpenPiConfig(
        host=args.host,
        port=args.port,
        device=args.device,
    ...
    # 创建OpenPi策略
    policy = OpenPiPolicy(config=openpi_config, device=device)
    
    # 运行机器人
    run_piper_with_openpi(
        robot=robot,
        policy=policy,
        device=device,
