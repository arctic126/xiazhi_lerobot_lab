...
def record(
    ...
    # 修改n_action_steps（如果有策略的话）
    if policy is not None:
        policy.config.n_action_steps = 30  # 设置为你想要的值
        policy.reset()  # 重新初始化动作队列
        print(f"Modified n_action_steps to: {policy.config.n_action_steps}")
        print(f"Action queue max length: {policy._action_queue.maxlen}")  # 这应该是 20
    ...
    while True:
        ...
        if not events["stop_recording"] and (
            ...
            # reset the policy to clean the action queue (just for inference)
            if policy is not None:  
                policy.reset()
