...
from lerobot.common.robot_devices.motors.configs import PiperRelMotorsBusConfig

class PiperRelMotorsBus:
    """
        对Piper SDK的二次封装
    """
    def __init__(self, 
                 config: PiperRelMotorsBusConfig):
        self.piper = C_PiperInterface_V2(config.can_name)
        self.piper.ConnectPort()
        self.motors = config.motors
        self.init_joint_position = [0.823, 0.0, 0.0, -1.152, 2.368, 7.309, 0.0] # [6 joints + 1 gripper] * 0.0 (0.001度)
        self.safe_disable_position = [0.823, 0.0, 0.0, -1.152, 2.368, 7.309, 0.0]
        self.pose_factor = 1000 # 单位 0.001mm
        self.joint_factor = 1000 # 1000*180/3.14， rad -> 度（单位0.001度）

        '''
        time stamp:1741874208.239119
        Hz:200.0
        ArmMsgJointFeedBack:
        Joint 1:-179, -0.179
        Joint 2:-1723, -1.723
    ...
    def read(self) -> Dict:
        ...
        return {
            ...
            "joint_6": joint_state.joint_6 / self.joint_factor,
             "gripper": 1.0 if gripper_state.grippers_angle > 30000 else 0.0  # 夹爪状态：0表示关闭，1表示打开
        }
