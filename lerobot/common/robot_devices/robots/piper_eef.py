...
class PiperEefRobot:
    def __init__(self, config: PiperEefRobotConfig | None = None, **kwargs):
        if config is None:
            config = PiperEefRobotConfig()
    ...
    def teleop_step(
        ...
        target_joints[0] += state['x']
        target_joints[1] += state['y']
        target_joints[2] += state['z']
        target_joints[3] += state['rx']
        target_joints[4] += state['ry']
        target_joints[5] += state['rz']
    ...
    def send_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        target_eef = action.tolist()
        self.arm.write(target_eef)
        # print('target_eef', target_eef)
        return action
