...
from lerobot.common.robot_devices.teleop.spacemouse import SpaceMouseController
...
from lerobot.common.robot_devices.robots.configs import FlexivRobotConfig
...
class FlexivRobot:
    def __init__(self, config: FlexivRobotConfig | None = None, **kwargs):
        ...
        # build piper motors
        self.flexiv_motors = make_motors_buses_from_configs(self.config.follower_arm)
        self.arm = self.flexiv_motors['main']
        
        # build gamepad teleop
        if not self.inference_time:
            self.teleop = SpaceMouseController()
        else:
            self.teleop = None
    ...
    def motor_features(self) -> dict:
        action_names = get_motor_names(self.flexiv_motors)
        state_names = get_motor_names(self.flexiv_motors)
        return {
            "action": {
                "dtype": "float32",
    ...
    def connect(self) -> None:
        ...
        if self.is_connected:
            raise RobotDeviceAlreadyConnectedError(
                "Flexiv is already connected. Do not run `robot.connect()` twice."
            )
        
        # connect piper
        self.arm.connect(enable=True)
        print("flexiv conneted")
    ...
    def disconnect(self) -> None:
        """move to home position, disenable flexiv and cameras"""
        # move flexiv to home position, disable
        if not self.inference_time:
            self.teleop.stop()

        # disconnect piper
        self.arm.safe_disconnect()
        print("flexiv disable after 5 seconds")
        time.sleep(5)
    ...
    def run_calibration(self):
        """move flexiv to the home position"""
    ...
    def send_action(self, action: torch.Tensor) -> torch.Tensor:
        """Write the predicted actions from policy to the motors
            For flexiv, we use delta eef pose control"""
        if not self.is_connected:
            raise RobotDeviceNotConnectedError(
                "Flexiv is not connected. You need to run `robot.connect()`."
            )

        # send to motors, torch to list
        target_tcp = action.tolist()
        self.arm.write(target_tcp)
    ...
    def capture_observation(self) -> dict:
        """capture current images and tcp positions"""
        if not self.is_connected:
            raise RobotDeviceNotConnectedError(
                "Flexiv is not connected. You need to run `robot.connect()`."
            )
