...
from lerobot.common.robot_devices.teleop.piper_leader import PiperLeader
...
from lerobot.common.robot_devices.robots.configs import Piper2RobotConfig

class Piper2Robot:
    def __init__(self, config: Piper2RobotConfig | None = None, **kwargs):
        if config is None:
            config = Piper2RobotConfig()
        ...
        if not self.inference_time:
            self.teleop = PiperLeader(self.config)
        else:
            self.teleop = None
    ...
    def teleop_step(
        ...
        if self.inference_time:  # read action from slave arm when teleoping for accuracy
            action = self.teleop.get_action() # target joint position from gamepad
        else:
            
            # !use time.sleep and read the slave arm joint position to get the true action
            time.sleep(1/30)
            action = self.arm.read()
        # print('action', action)
